...
module SyncIO = struct
  ...
  let get_operation request =
    ...
    with Not_found -> fun connection ->
                      failwith ("Operation not found: <" ^ name ^ ">")
...
module AsyncIO = struct
  ...
  let read_response channel =
    AsyncUtils.fail "Unsupported operation JsonProtocol.AsyncIO.read_response"
  let write_response channel v =
    AsyncUtils.fail "Unsupported operation JsonProtocol.AsyncIO.write_response"
