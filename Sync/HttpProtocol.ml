...
module Header =
  struct
    ...
    let () = assert (date ~timestamp:1453872218. () = (Date, "Wed, 27 Jan 2016 05:23:38 GMT"))
    let () =
      assert begin
	  try let aux timestamp = (date ~timestamp ()) |> snd |> parse_date in
		...
		if t <> aux t
		then failwith (((date ~timestamp:t ()) |> snd) ^ " vs. " ^ ((date ~timestamp:(aux t) ()) |> snd))
	      done;
	      true
  ...
  struct
    ...
    let parse ?prefix channel =
	  ...
	  let request_http_version =
	    let v = String.sub start_line protocol_start (String.length start_line - protocol_start) in
	    HttpVersion.parse v in
	  let http_method = Method.parse (String.sub start_line 0 (target_start-1)) in
	  let request_target = String.sub start_line target_start (protocol_start - target_start - 1)
...
module Headers =
  struct
	...
	then begin
	    ...
	    let header_name = Header.parse (String.trim (String.sub header_line 0 colon))
	    and header_value = String.trim (String.sub header_line
						       (colon + 1)
...
module Body =
  struct
	...
	if !remaining = 0
	then if chunked
	     then Lwt_io.read ~count:(String.length crlf) channel >>= fun _ -> Lwt.return_none
...
module HttpRequest =
  struct
    ...
    let parse ?prefix channel =
      ...
      >>= fun request_headers -> let request_body = Body.parse channel request_headers in
...
module HttpResponse =
  struct
    ...
    let parse ?prefix channel =
      ...
      >>= fun response_headers -> let response_body = Body.parse channel response_headers in
...
let modules =
  let letters =
    Array.fold_left (fun letters m ->
		    ...
		    Method.all
  in List.rev_map
       (fun prefix -> let module P = Protocol.Make(IO(struct let prefix = 'G' end))(ExnWrapper)
